...
Value *SimplifyDxilCall(llvm::Function *F, ArrayRef<Value *> Args,
  ...
  case DXIL::OpCode::VectorReduceOr: {
    // VectorReduce* opcodes take (opcode, vector). When the vector
    // operand is a constant, fold to the bitwise reduction across all
    // elements. This is important because in SM 6.9 native vectors
    // survive into validation: an unfolded VectorReduce keeping a loop
    // guard live can prevent the loop from being deleted and trigger
    // "Loop must have break" validation errors.
