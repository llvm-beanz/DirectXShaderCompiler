...
STDMETHODIMP dxil_debug_info::DxcPixDxilDebugInfo::IsVariableInRegister(
    _In_ DWORD InstructionOffset, _In_ const wchar_t *VariableName) {
  return S_OK;
}

STDMETHODIMP dxil_debug_info::DxcPixDxilDebugInfo::GetFunctionName(
    _In_ DWORD InstructionOffset, _Outptr_result_z_ BSTR *ppFunctionName) {
...
STDMETHODIMP dxil_debug_info::DxcPixDxilDebugInfo::GetStackDepth(
    _In_ DWORD InstructionOffset, _Outptr_ DWORD *StackDepth) {
  llvm::Instruction *IP = FindInstruction(InstructionOffset);
...
STDMETHODIMP_(DWORD)
dxil_debug_info::DxcPixDxilSourceLocations::GetColumnByIndex(_In_ DWORD Index) {
  if (Index >= static_cast<DWORD>(m_locations.size())) {
    return E_BOUNDS;
  }
...
STDMETHODIMP_(DWORD)
dxil_debug_info::DxcPixDxilSourceLocations::GetLineNumberByIndex(
    _In_ DWORD Index) {
  if (Index >= static_cast<DWORD>(m_locations.size())) {
    return E_BOUNDS;
  }
