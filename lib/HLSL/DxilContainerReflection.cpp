...
public:
  ...
  HRESULT STDMETHODCALLTYPE GetPartKind(UINT32 idx,
                                        _Out_ UINT32 *pResult) override;
  HRESULT STDMETHODCALLTYPE
  GetPartContent(UINT32 idx, _COM_Outptr_ IDxcBlob **ppResult) override;
  HRESULT STDMETHODCALLTYPE FindFirstPartKind(UINT32 kind,
                                              _Out_ UINT32 *pResult) override;
  HRESULT STDMETHODCALLTYPE GetPartReflection(
      UINT32 idx, REFIID iid, _COM_Outptr_ void **ppvObject) override;
};
...
public:
  ...
  GetConstantBufferByIndex(THIS_ _In_ UINT Index) override;
  STDMETHODIMP_(ID3D12ShaderReflectionConstantBuffer *)
  GetConstantBufferByName(THIS_ _In_ LPCSTR Name) override;

  STDMETHODIMP
  GetResourceBindingDesc(THIS_ _In_ UINT ResourceIndex,
                         _Out_ D3D12_SHADER_INPUT_BIND_DESC *pDesc) override;

  STDMETHODIMP
  GetInputParameterDesc(THIS_ _In_ UINT ParameterIndex,
                        _Out_ D3D12_SIGNATURE_PARAMETER_DESC *pDesc) override;
  STDMETHODIMP
  GetOutputParameterDesc(THIS_ _In_ UINT ParameterIndex,
                         _Out_ D3D12_SIGNATURE_PARAMETER_DESC *pDesc) override;
  ...
  GetVariableByName(THIS_ _In_ LPCSTR Name) override;
  ...
  STDMETHODIMP
  GetMinFeatureLevel(THIS_ _Out_ enum D3D_FEATURE_LEVEL *pLevel) override;

  STDMETHODIMP_(UINT)
  GetThreadGroupSize(THIS_ _Out_opt_ UINT *pSizeX, _Out_opt_ UINT *pSizeY,
...
public:
  ...
  (THIS_ _In_ INT FunctionIndex) override;
};
...
DxilContainerReflection::Load(IDxcBlob *pContainer) {
...
_Use_decl_annotations_ HRESULT
DxilContainerReflection::GetPartCount(UINT32 *pResult) {
  if (pResult == nullptr)
    return E_POINTER;
  if (!IsLoaded())
...
_Use_decl_annotations_ HRESULT
DxilContainerReflection::GetPartKind(UINT32 idx, _Out_ UINT32 *pResult) {
  if (pResult == nullptr)
    return E_POINTER;
  if (!IsLoaded())
    return E_NOT_VALID_STATE;
...
_Use_decl_annotations_ HRESULT DxilContainerReflection::GetPartContent(
    UINT32 idx, _COM_Outptr_ IDxcBlob **ppResult) {
  if (ppResult == nullptr)
    return E_POINTER;
  *ppResult = nullptr;
  if (!IsLoaded())
...
_Use_decl_annotations_ HRESULT
DxilContainerReflection::FindFirstPartKind(UINT32 kind, _Out_ UINT32 *pResult) {
  if (pResult == nullptr)
    return E_POINTER;
  *pResult = 0;
  if (!IsLoaded())
...
_Use_decl_annotations_ HRESULT DxilContainerReflection::GetPartReflection(
    UINT32 idx, REFIID iid, void **ppvObject) {
...
STDMETHODIMP
CShaderReflectionType::IsEqual(THIS_ ID3D12ShaderReflectionType *pType) {
  // TODO: implement this check, if users actually depend on it
  return S_FALSE;
}

STDMETHODIMP_(ID3D12ShaderReflectionType *)
CShaderReflectionType::GetSubType(THIS) {
  // TODO: implement `class`-related features, if requested
  return nullptr;
}

STDMETHODIMP_(ID3D12ShaderReflectionType *)
CShaderReflectionType::GetBaseClass(THIS) {
  // TODO: implement `class`-related features, if requested
  return nullptr;
}
...
STDMETHODIMP_(ID3D12ShaderReflectionType *)
CShaderReflectionType::GetInterfaceByIndex(THIS_ UINT uIndex) {
  // HLSL interfaces have been deprecated
  return nullptr;
}

STDMETHODIMP
CShaderReflectionType::IsOfType(THIS_ ID3D12ShaderReflectionType *pType) {
  // TODO: implement `class`-related features, if requested
  return S_FALSE;
}

STDMETHODIMP CShaderReflectionType::ImplementsInterface(
    THIS_ ID3D12ShaderReflectionType *pBase) {
  // HLSL interfaces have been deprecated
  return S_FALSE;
}
...
_Use_decl_annotations_ HRESULT
DxilShaderReflection::GetDesc(D3D12_SHADER_DESC *pDesc) {
  if (nullptr == pDesc)
    return E_POINTER;
...
_Use_decl_annotations_ ID3D12ShaderReflectionConstantBuffer *
...
_Use_decl_annotations_ HRESULT DxilShaderReflection::GetResourceBindingDesc(
    UINT ResourceIndex, _Out_ D3D12_SHADER_INPUT_BIND_DESC *pDesc) {
...
_Use_decl_annotations_ HRESULT DxilShaderReflection::GetInputParameterDesc(
    UINT ParameterIndex, _Out_ D3D12_SIGNATURE_PARAMETER_DESC *pDesc) {
  IFRBOOL(pDesc != nullptr, E_INVALIDARG);
...
_Use_decl_annotations_ HRESULT DxilShaderReflection::GetOutputParameterDesc(
...
_Use_decl_annotations_ ID3D12ShaderReflectionVariable *
...
_Use_decl_annotations_ HRESULT
DxilShaderReflection::GetResourceBindingDescByName(
...
_Use_decl_annotations_ UINT DxilShaderReflection::GetThreadGroupSize(
    UINT *pSizeX, UINT *pSizeY, UINT *pSizeZ) {
...
_Use_decl_annotations_ HRESULT
DxilLibraryReflection::GetDesc(D3D12_LIBRARY_DESC *pDesc) {
  IFR(ZeroMemoryToOut(pDesc));
...
_Use_decl_annotations_ ID3D12FunctionReflection *
