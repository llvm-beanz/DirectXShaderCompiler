...
static QualType GetOrCreateMatrixSpecialization(
    ASTContext &context, Sema *sema, _In_ ClassTemplateDecl *matrixTemplateDecl,
...
static FunctionDecl *
AddHLSLIntrinsicFunction(ASTContext &context, _In_ NamespaceDecl *NS,
                         LPCSTR tableName, LPCSTR lowering,
...
/// operator.
/// </summary>
static bool IsExpressionBinaryComma(_In_ const Expr *expr) {
...
/// </summary>
static void
SilenceSequenceDiagnostics(_Inout_ InitializationSequence *initSequence) {
...
template <typename T> inline void AssignOpt(T value, _Out_opt_ T *ptr) {
  if (ptr != nullptr) {
    *ptr = value;
  }
}
...
static void GetIntrinsicMethods(ArBasicKind kind,
                                _Outptr_result_buffer_(*intrinsicCount)
...
private:
  ...
  QualType AddTemplateParamToArray(
      _In_z_ const char *name, _Inout_ CXXRecordDecl *recordDecl,
      int templateDepth,
      _Inout_count_c_(g_MaxIntrinsicParamCount + 1)
    ...
    _Analysis_assume_(*templateParamNamedDeclsCount <
                      _countof(templateParamNamedDecls));
  ...
  void AddObjectIntrinsicTemplate(_Inout_ CXXRecordDecl *recordDecl,
                                  int templateDepth,
  ...
  void AddObjectMethods(ArBasicKind kind, _In_ CXXRecordDecl *recordDecl,
                        int templateDepth) {
    DXASSERT_NOMSG(recordDecl != nullptr);
  ...
  void AddDoubleSubscriptSupport(
      _In_ ClassTemplateDecl *typeDecl, _In_ CXXRecordDecl *recordDecl,
      _In_z_ const char *memberName, QualType elementType,
      TemplateTypeParmDecl *templateTypeParmDecl, _In_z_ const char *type0Name,
      _In_z_ const char *type1Name, _In_z_ const char *indexer0Name,
      QualType indexer0Type, _In_z_ const char *indexer1Name,
      QualType indexer1Type) {
  ...
  FunctionDecl *
  AddSubscriptSpecialization(_In_ FunctionTemplateDecl *functionTemplate,
...
public:
  ...
  QualType NewSimpleAggregateType(_In_ ArTypeObjectKind ExplicitKind,
                                  _In_ ArBasicKind componentType,
                                  _In_ UINT64 qwQual, _In_ UINT uRows,
                                  _In_ UINT uCols) {
  ...
  bool MatchArguments(const IntrinsicDefIter &cursor,
                      _In_ QualType objectElement,
                      _In_ QualType functionTemplateTypeArg,
                      _In_ ArrayRef<Expr *> Args, _Out_ std::vector<QualType> *,
  ...
  bool IsValidObjectElement(LPCSTR tableName, _In_ IntrinsicOp op,
                            _In_ QualType objectElement);

  // Returns the iterator with the first entry that matches the requirement
  IntrinsicDefIter FindIntrinsicByNameAndArgCount(
      _In_count_(tableSize) const HLSL_INTRINSIC *table, size_t tableSize,
      StringRef typeName, StringRef nameIdentifier, size_t argumentCount) {
  ...
  bool ValidateCast(SourceLocation Loc, _In_ Expr *source, QualType target,
  ...
  clang::QualType CheckVectorConditional(_In_ ExprResult &Cond,
                                         _In_ ExprResult &LHS,
                                         _In_ ExprResult &RHS,
                                         _In_ SourceLocation QuestionLoc);

  clang::QualType ApplyTypeSpecSignToParsedType(_In_ clang::QualType &type,
  ...
  FindStructBasicType(_In_ DeclContext *functionDeclContext);
  ...
  void FindIntrinsicTable(_In_ DeclContext *functionDeclContext,
  ...
  /// initialize.</param>
  void
  InitializeInitSequenceForHLSL(const InitializedEntity &Entity,
  ...
  clang::Expr *HLSLImpCastToScalar(_In_ clang::Sema *self,
                                   _In_ clang::Expr *From,
                                   ArTypeObjectKind FromShape,
                                   ArBasicKind EltKind);
  clang::ExprResult
  PerformHLSLConversion(_In_ clang::Expr *From, _In_ clang::QualType targetType,
  ...
  TrySubscriptIndexInitialization(_In_ clang::Expr *SrcExpr,
  ...
  FunctionDecl *AddHLSLIntrinsicMethod(
      LPCSTR tableName, LPCSTR lowering, _In_ const HLSL_INTRINSIC *intrinsic,
      _In_ FunctionTemplateDecl *FunctionTemplate, ArrayRef<Expr *> Args,
...
private:
  ...
  bool pushTrackerForType(QualType type,
                          _In_opt_ MultiExprArg::iterator expression);
...
FunctionDecl *HLSLExternalSource::AddSubscriptSpecialization(
    _In_ FunctionTemplateDecl *functionTemplate, QualType objectElement,
    const FindStructBasicTypeResult &findResult) {
  DXASSERT_NOMSG(functionTemplate != nullptr);
...
_Use_decl_annotations_ bool HLSLExternalSource::MatchArguments(
    const IntrinsicDefIter &cursor, QualType objectElement,
...
_Use_decl_annotations_ HLSLExternalSource::FindStructBasicTypeResult
...
_Use_decl_annotations_ void HLSLExternalSource::FindIntrinsicTable(
    DeclContext *functionDeclContext, const char **name,
    const HLSL_INTRINSIC **intrinsics, size_t *intrinsicCount) {
  DXASSERT_NOMSG(functionDeclContext != nullptr);
  DXASSERT_NOMSG(name != nullptr);
  DXASSERT_NOMSG(intrinsics != nullptr);
  DXASSERT_NOMSG(intrinsicCount != nullptr);
...
HLSLExternalSource::CollectInfo(QualType type, ArTypeInfo *pTypeInfo) {
...
TryParseMatrixMemberAccess(_In_z_ const char *memberText,
                           _Out_ MatrixMemberAccessPositions *value) {
  DXASSERT_NOMSG(memberText != nullptr);
  DXASSERT_NOMSG(value != nullptr);
...
TryParseVectorMemberAccess(_In_z_ const char *memberText,
                           _Out_ VectorMemberAccessPositions *value) {
  DXASSERT_NOMSG(memberText != nullptr);
  DXASSERT_NOMSG(value != nullptr);
...
clang::Expr *HLSLExternalSource::HLSLImpCastToScalar(_In_ clang::Sema *self,
                                                     _In_ clang::Expr *From,
                                                     ArTypeObjectKind FromShape,
                                                     ArBasicKind EltKind) {
...
clang::ExprResult HLSLExternalSource::PerformHLSLConversion(
    _In_ clang::Expr *From, _In_ clang::QualType targetType,
    _In_ const clang::StandardConversionSequence &SCS,
    _In_ clang::Sema::CheckedConversionKind CCK) {
...
static bool HandleVoidConversion(QualType source, QualType target,
                                 bool explicitConversion, _Out_ bool *allowed) {
...
_Use_decl_annotations_ bool HLSLExternalSource::CanConvert(
    SourceLocation loc, Expr *sourceExpr, QualType target,
    bool explicitConversion, _Out_opt_ TYPE_CONVERSION_REMARKS *remarks,
    _Inout_opt_ StandardConversionSequence *standard) {
  UINT uTSize, uSSize;
...
clang::QualType HLSLExternalSource::CheckVectorConditional(
    _In_ ExprResult &Cond, _In_ ExprResult &LHS, _In_ ExprResult &RHS,
    _In_ SourceLocation QuestionLoc) {
...
bool HLSLExternalSource::TryStaticCastForHLSL(
    ...
    bool SuppressErrors, _Inout_opt_ StandardConversionSequence *standard) {
...
static bool IsPrecisionLossIrrelevant(ASTContext &Ctx,
                                      _In_ const Expr *sourceExpr,
                                      QualType targetType,
...
bool HLSLExternalSource::ValidateCast(
    SourceLocation OpLoc, _In_ Expr *sourceExpr, QualType target,
    bool explicitConversion, bool suppressWarnings, bool suppressErrors,
    _Inout_opt_ StandardConversionSequence *standard) {
  DXASSERT_NOMSG(sourceExpr != nullptr);
...
unsigned
hlsl::CaculateInitListArraySizeForHLSL(_In_ clang::Sema *sema,
...
bool hlsl::IsConversionToLessOrEqualElements(
    _In_ clang::Sema *self, const clang::ExprResult &sourceExpr,
...
clang::ExprResult hlsl::MaybeConvertMemberAccess(_In_ clang::Sema *self,
                                                 _In_ clang::Expr *E) {
  return HLSLExternalSource::FromSema(self)->MaybeConvertMemberAccess(E);
}

bool hlsl::TryStaticCastForHLSL(
    _In_ Sema *self, ExprResult &SrcExpr, QualType DestType,
...
clang::ExprResult
hlsl::PerformHLSLConversion(_In_ clang::Sema *self, _In_ clang::Expr *From,
...
hlsl::TrySubscriptIndexInitialization(_In_ clang::Sema *self,
...
bool hlsl::IsMatrixType(_In_ clang::Sema *self, _In_ clang::QualType type) {
...
/// <param name="self">Sema with context.</param>
/// <param name="type">QualType to check.</param>
bool hlsl::IsVectorType(_In_ clang::Sema *self, _In_ clang::QualType type) {
  return HLSLExternalSource::FromSema(self)->GetTypeObjectKind(type) ==
         AR_TOBJ_VECTOR;
}
...
clang::QualType
hlsl::GetOriginalMatrixOrVectorElementType(_In_ clang::QualType type) {
...
clang::QualType hlsl::GetOriginalElementType(_In_ clang::Sema *self,
...
bool hlsl::IsObjectType(_In_ clang::Sema *self, _In_ clang::QualType type,
...
bool hlsl::CanConvert(_In_ clang::Sema *self, clang::SourceLocation loc,
...
void hlsl::RegisterIntrinsicTable(_In_ clang::ExternalSemaSource *self,
                                  _In_ IDxcIntrinsicTable *table) {
  DXASSERT_NOMSG(self != nullptr);
  DXASSERT_NOMSG(table != nullptr);
...
clang::QualType hlsl::CheckVectorConditional(
    _In_ clang::Sema *self, _In_ clang::ExprResult &Cond,
    _In_ clang::ExprResult &LHS, _In_ clang::ExprResult &RHS,
