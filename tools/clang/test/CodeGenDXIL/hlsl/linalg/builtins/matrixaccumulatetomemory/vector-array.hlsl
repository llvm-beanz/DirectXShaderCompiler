...
// DXIL operation is still overloaded on the scalar type, so the array is
// indexed down to its first scalar element.
...
void main() {
  ...
  // CHECK-SAME: %dx.types.LinAlgMatrixC4M5N4U1S2 %{{.*}}, float addrspace(3)* getelementptr inbounds ([64 x <2 x float>],
  // CHECK-SAME: [64 x <2 x float>] addrspace(3)* @"\01?SharedArr@@3PAV?$vector@M$01@@A", i32 0, i32 0, i32 0),
