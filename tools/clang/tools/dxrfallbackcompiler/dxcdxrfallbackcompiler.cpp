...
static HRESULT FindDxilProgram(IDxcBlob *pBlob, _In_ DxilFourCC FourCC,
...
public:
  ...
  __override HRESULT STDMETHODCALLTYPE PatchShaderBindingTables(
      _In_ const LPCWSTR pEntryName, _In_ DxcShaderBytecode *pShaderBytecode,
      void *pShaderInfo, _COM_Outptr_ IDxcOperationResult **ppResult);

  __override HRESULT STDMETHODCALLTYPE RenameAndLink(
      _In_count_(libCount) DxcShaderBytecode *pLibs, UINT32 libCount,
      _In_count_(ExportCount) DxcExportDesc *pExports, UINT32 ExportCount,
      _COM_Outptr_ IDxcOperationResult **ppResult);

  __override HRESULT STDMETHODCALLTYPE Compile(
      _In_count_(libCount) DxcShaderBytecode *pLibs, UINT32 libCount,
      _In_count_(shaderCount) const LPCWSTR *pShaderNames,
      _Out_writes_(shaderCount) DxcShaderInfo *pShaderInfo, UINT32 shaderCount,
      UINT32 maxAttributeSize, _COM_Outptr_ IDxcOperationResult **ppResult);

  __override HRESULT STDMETHODCALLTYPE
  Link(_In_ const LPCWSTR pEntryName, _In_count_(libCount) IDxcBlob **pLibs,
       UINT32 libCount, _In_count_(shaderCount) const LPCWSTR *pShaderNames,
       _In_count_(shaderCount) DxcShaderInfo *pShaderInfo, UINT32 shaderCount,
...
HRESULT STDMETHODCALLTYPE DxcDxrFallbackCompiler::Link(
    _In_ const LPCWSTR pEntryName, _In_count_(libCount) IDxcBlob **pLibs,
    UINT32 libCount, _In_count_(shaderCount) const LPCWSTR *pShaderNames,
    _In_count_(shaderCount) DxcShaderInfo *pShaderInfo, UINT32 shaderCount,
    UINT32 maxAttributeSize, UINT32 stackSizeInBytes,
    _COM_Outptr_ IDxcOperationResult **ppResult) {
...
HRESULT STDMETHODCALLTYPE DxcDxrFallbackCompiler::Compile(
    _In_count_(libCount) DxcShaderBytecode *pShaderLibs, UINT32 libCount,
    _In_count_(shaderCount) const LPCWSTR *pShaderNames,
    _Out_writes_(shaderCount) DxcShaderInfo *pShaderInfo, UINT32 shaderCount,
    UINT32 maxAttributeSize, _COM_Outptr_ IDxcOperationResult **ppResult) {
