...
// ---------------------------------------------------------------------------
// Auto-diff generation (consumed by -generate-differentials)
// ---------------------------------------------------------------------------
...
public:
  ...
  AutoDiffEmitter(Mode M, llvm::StringRef ElemType, raw_ostream &OS,
...
private:
  ...
  void emitCall(const CallExpr *CE) {
    ...
    llvm::StringRef Name = Callee ? Callee->getName() : "";
    if (M == Fwd) {
...
static void emitAutoDiffSignature(const FunctionDecl *FD, AutoDiffEmitter::Mode M,
                                  llvm::StringRef ElemType, raw_ostream &OS) {
...
static bool emitAutoDiffFunction(const FunctionDecl *FD,
  ...
  std::string ElemType;
  llvm::raw_string_ostream ES(ElemType);
...
static void EmitAutoDiffForFunction(const FunctionDecl *FD,
...
// Walks a translation unit, printing each top-level declaration normally and
// emitting auto-diff stubs immediately after any function carrying the
// HLSLAutoDiffAttr.
static void PrintTranslationUnitWithDifferentials(TranslationUnitDecl *tu,
                                                  raw_ostream &OS,
...
static HRESULT DoSimpleReWrite(DxcLangExtensionsHelper *pHelper,
  ...
  } else if (opts.RWOpt.GenerateDifferentials) {
    PrintTranslationUnitWithDifferentials(tu, o, p);
  } else {
    tu->print(o, p);
  }
