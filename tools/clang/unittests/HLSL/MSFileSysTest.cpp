...
public:
  FixedEnumSTATSTG(_In_count_(itemCount) const STATSTG *items,
                   unsigned itemCount) {
    m_dwRef = 0;
    m_index = 0;
  ...
  virtual HRESULT STDMETHODCALLTYPE Next(ULONG celt,
                                         _Out_writes_to_(celt, *pceltFetched)
                                             STATSTG *rgelt,
...
public:
  ...
  OpenStorage(_In_ LPCWSTR lpFileName, _In_ DWORD dwDesiredAccess,
              _In_ DWORD dwShareMode, _In_ DWORD dwCreationDisposition,
              _In_ DWORD dwFlagsAndAttributes, IUnknown **pResult) override {
  ...
  virtual HRESULT STDMETHODCALLTYPE SetStorageTime(
      _In_ IUnknown *storage, _In_opt_ const FILETIME *lpCreationTime,
      _In_opt_ const FILETIME *lpLastAccessTime,
      _In_opt_ const FILETIME *lpLastWriteTime) override {
    return E_NOTIMPL;
  }
  virtual HRESULT STDMETHODCALLTYPE GetFileInformationForStorage(
      _In_ IUnknown *storage,
      _Out_ LPBY_HANDLE_FILE_INFORMATION lpFileInformation) override {
    return E_NOTIMPL;
  }
  virtual HRESULT STDMETHODCALLTYPE GetFileTypeForStorage(
      _In_ IUnknown *storage, _Out_ DWORD *fileType) override {
    return E_NOTIMPL;
  }
  virtual HRESULT STDMETHODCALLTYPE CreateHardLinkInStorage(
      _In_ LPCWSTR lpFileName, _In_ LPCWSTR lpExistingFileName) override {
    return E_NOTIMPL;
  }
  virtual HRESULT STDMETHODCALLTYPE MoveStorage(_In_ LPCWSTR lpExistingFileName,
                                                _In_opt_ LPCWSTR lpNewFileName,
                                                _In_ DWORD dwFlags) override {
    return E_NOTIMPL;
  }
  virtual HRESULT STDMETHODCALLTYPE GetFileAttributesForStorage(
      _In_ LPCWSTR lpFileName, _Out_ DWORD *pResult) override {
    return E_NOTIMPL;
  }
  virtual HRESULT STDMETHODCALLTYPE
  DeleteStorage(_In_ LPCWSTR lpFileName) override {
    return E_NOTIMPL;
  }
  virtual HRESULT STDMETHODCALLTYPE
  ...
  virtual HRESULT STDMETHODCALLTYPE
  CreateDirectoryStorage(_In_ LPCWSTR lpPathName) override {
    return E_NOTIMPL;
  }
  virtual HRESULT STDMETHODCALLTYPE GetCurrentDirectoryForStorage(
      DWORD nBufferLength, _Out_writes_(nBufferLength) LPWSTR lpBuffer,
      _Out_ DWORD *len) override {
    return E_NOTIMPL;
  }
  ...
  virtual HRESULT STDMETHODCALLTYPE GetTempStoragePath(
      DWORD nBufferLength, _Out_writes_(nBufferLength) LPWSTR lpBuffer,
      _Out_ DWORD *len) override {
    return E_NOTIMPL;
  }
  virtual HRESULT STDMETHODCALLTYPE
  SupportsCreateSymbolicLink(_Out_ BOOL *pResult) override {
    return E_NOTIMPL;
  }
  virtual HRESULT STDMETHODCALLTYPE CreateSymbolicLinkInStorage(
      _In_ LPCWSTR lpSymlinkFileName, _In_ LPCWSTR lpTargetFileName,
      DWORD dwFlags) override {
    return E_NOTIMPL;
  }
  virtual HRESULT STDMETHODCALLTYPE CreateStorageMapping(
      _In_ IUnknown *hFile, _In_ DWORD flProtect, _In_ DWORD dwMaximumSizeHigh,
      _In_ DWORD dwMaximumSizeLow, _Outptr_ IUnknown **pResult) override {
    return E_NOTIMPL;
  }
  virtual HRESULT MapViewOfFile(_In_ IUnknown *hFileMappingObject,
  ...
  virtual HRESULT STDMETHODCALLTYPE
  OpenStdStorage(int standardFD, _Outptr_ IUnknown **pResult) override {
    return E_NOTIMPL;
  }
  virtual HRESULT STDMETHODCALLTYPE
  GetStreamDisplay(_COM_Outptr_result_maybenull_ ITextFont **textFont,
                   _Out_ unsigned *columnCount) override {
    return E_NOTIMPL;
  }
};
