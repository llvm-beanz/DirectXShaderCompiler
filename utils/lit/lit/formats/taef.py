...
class TAEFTest(TestFormat):
    def __init__(self, test_sub_dir, test_suffix):
        ...
        self.test_suffix += '.dll'

    def getTAEFTests(self, path, litConfig, localConfig):
        """getTAEFTests(path) - [name]

        Return the tests available in TAEF library.

        Args:
          path: String path to a TAEF library
          litConfig: LitConfig instance
          localConfig: TestingConfig instance"""

        try:
            lines = lit.util.capture(['te.exe', path, '/list'],
                                     env=localConfig.environment)
            lines = lines.split('\r\n')
        except:
            litConfig.error("unable to discover taef tests in %r" % path)
            raise StopIteration
